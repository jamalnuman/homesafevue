  ...
  <body>
    ...
    <div id="vue-app-one">
      <input type="text" ref="input"/>
        the 'ref' tag is need and the name of it can be whatever. now it can be accessed in the Vue instance above
...
<!-- Lesson 19 - Importing and Nesting Components
 ...
 <!-- Lesson 21 - Components Example STYLES ARE NOT INCLUDED
  ...
  HEADER COMPONENT
  <template>
    <header> header is the root tag, not <div> cause this is the header component
      <h1>{{ title }}</h1>
    </header>
  </template>
  <script>
...
FOOTER COMPONENT
<template>
  <footer> footer is the root tag, not <div>, cause this is the footer component
    <p>{{ copyright }}</p>
  </footer>
</template>
<script>
...
<!-- Lesson 24 - Events
  An event occurs in the child component, the parent component is listening for that specific event ($emit) and then the parent performs a specific action and re-renders it's props to the child components, which then updates the children components. similar to reference type actions but based upon a primitive event occuring.  
...
export default {
  ...
  methods: {
    changeTitle: function(){
      ...
      the first argument in $emit is the name of the event that is $emit executes, the second is the data the the parent component is expecting. 
    }
  }
...
<!-- Lesson 26 - Life Cycle Hooks
  ...
  Here no data in the data hash is available for use. the data and events have not be created. Plus nothing is seen on the screen, cause the DOM has not been mounted (HMTL and CSS has not been applied to the page)
...
beforeUpdate(){
  ...
  Once the DOM has been updated. Meaning the webpage being viewed is updated with the change..any of the CRUD changes. 
...
<!-- Lesson 27 - Slots 
  These are used to pass entire HTML templates to child nodes. The HTML template structure goes in-btw <form-helper> tags. 

APP.VUE
<template>
  <div>
    <form-helper>
      <div slot="form-header"> this how you name the slots to define which slots to place where on the page 
...
<template>
  <div>
    <keep-alive> this is to keep the fields with the data that was entered once left and came back to it 
